...
export async function scanSharing(creds, sites, opts = {}, progress = () => {}) {
  ...
  const fail = (siteName, err) => {
    sitesFailed++;
    if (errors.length < 8) errors.push(`${siteName}: ${err.message.slice(0, 160)}`);
  };

  for (let done = 0; done < targets.length; done++) {
    const site = targets[done];
    progress({ phase: 'scan', site: site.name, done, total: targets.length, links: links.length, itemsScanned, failed: sitesFailed });
    ...
    try {
      ...
      for (const drive of drives) {
        try {
          ...
          if (!siteFailed) { siteFailed = true; fail(`${site.name} (${drive.name})`, err); }
  ...
  const result = { links, sitesScanned: targets.length, sitesFailed, itemsScanned, truncatedDrives, errors };
  progress({ phase: 'done', done: targets.length, total: targets.length, links: links.length, itemsScanned, failed: sitesFailed });
  return result;
}
