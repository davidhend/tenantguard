...
// Certificate credentials: OAuth2 client assertion — a short-lived JWT signed
// with the app's private key. Required by SharePoint app-only; Graph accepts
// it too. The x5t header carries the certificate's SHA-1 thumbprint so Entra
// can match the uploaded public cert.
function clientAssertion({ tenantId, clientId, certificate, privateKey }) {
  const der = Buffer.from(certificate.replace(/-----[^-]+-----/g, '').replace(/\s+/g, ''), 'base64');
  const now = Math.floor(Date.now() / 1000);
  const header = { alg: 'RS256', typ: 'JWT', x5t: b64u(createHash('sha1').update(der).digest()) };
  const payload = {
    aud: `https://login.microsoftonline.com/${tenantId}/oauth2/v2.0/token`,
    iss: clientId, sub: clientId, jti: randomUUID(), nbf: now - 60, exp: now + 600,
  };
...
// Uses certificate credentials when present, client secret otherwise.
export function makeAuth(creds, scope = 'https://graph.microsoft.com/.default') {
  let token = null;
  let expiresAt = 0;
  return async (forceRefresh = false) => {
    if (forceRefresh || !token || Date.now() > expiresAt - 5 * 60 * 1000) {
      const params = { client_id: creds.clientId, scope, grant_type: 'client_credentials' };
      if (creds.certificate && creds.privateKey) {
        params.client_assertion_type = 'urn:ietf:params:oauth:client-assertion-type:jwt-bearer';
        params.client_assertion = clientAssertion(creds);
      } else {
...
// Generic authenticated JSON request with 401-refresh and 429/503 retry.
// Relative URLs hit Graph v1.0; absolute URLs (e.g. SharePoint admin) pass through.
export async function httpJson(auth, url, opts = {}, attempt = 0) {
  ...
  const res = await fetch(url.startsWith('http') ? url : GRAPH + url, { method: opts.method ?? 'GET', headers, body });
  // Token expired mid-run — refresh once and retry.
  if (res.status === 401 && attempt < 2) {
    await auth(true);
    return httpJson(auth, url, opts, attempt + 1);
  }
  // Graph and SharePoint throttle aggressively on large tenants — honor Retry-After.
  if (res.status === 429 || res.status === 503) {
    if (attempt >= 6) throw new Error(`Throttled repeatedly on ${url}`);
    const wait = Number(res.headers.get('retry-after')) || 2 ** attempt;
    opts.onThrottle?.(wait); // let long-running jobs surface the backoff
    await new Promise(r => setTimeout(r, wait * 1000));
    return httpJson(auth, url, opts, attempt + 1);
  }
  if (!res.ok) {
    const text = await res.text();
    throw new Error(`${opts.method ?? 'GET'} ${url} failed (${res.status}): ${text.slice(0, 300)}`);
  }
...
const get = (auth, url) => httpJson(auth, url);
...
async function getAll(auth, url, cap = 200000, opts = {}) {
  const items = [];
  let next = url;
  while (next && items.length < cap) {
    const page = await httpJson(auth, next, opts);
...
export async function syncTenant(creds, log = () => {}) {
  ...
  // Tenant-wide sharing ceiling. Per-site sharing settings are NOT exposed by
  // Graph (they need the SharePoint Admin API + certificate auth), so sites
  // get evidence-based values below instead.
  // Requires SharePointTenantSettings.Read.All (optional).
  let tenantSharing = null;
  try {
    const sp = await get(auth, '/admin/sharepoint/settings');
    tenantSharing = sp.sharingCapability ?? null;
  } catch (err) {
    log(`Tenant sharing settings unavailable (grant SharePointTenantSettings.Read.All to enable): ${err.message.slice(0, 100)}`);
  }
  ...
  const sites = rawSites
    ...
    .map((s, i) => ({
      ...
      externalSharing: 'none-observed', // upgraded below (guests) and by the sharing scan (anyone links)
      storageMB: null,
  ...
  // Evidence-based external sharing: a guest with access proves the site
  // shares externally today, regardless of what its settings would allow.
  const guestSites = new Set(permissions.filter(p => p.source === 'Guest access').map(p => p.siteId));
  for (const site of sites) {
    if (guestSites.has(site.id)) site.externalSharing = 'existing-guests';
  }

  log('Done.');
  return { tenantName, tenantSharing, users, groups, sites, links: [], permissions };
}
...
export async function scanSharing(creds, sites, opts = {}, progress = () => {}) {
  ...
  // Observability: which sites are being walked right now, and whether
  // Microsoft is throttling us (Retry-After sleeps are otherwise invisible
  // and make a parallel scan look stalled or sequential).
  const active = new Set();
  let throttleUntil = 0;
  const reqOpts = { onThrottle: (wait) => { throttleUntil = Math.max(throttleUntil, Date.now() + wait * 1000); } };
  ...
  const report = (siteName) => progress({
    ...
    throttledFor: Math.max(0, Math.ceil((throttleUntil - Date.now()) / 1000)),
  });
  const flush = () => {
  ...
  await pool(targets, concurrency, async (site) => {
    ...
    try {
      const drives = await getAll(auth, `/sites/${site.id}/drives?$select=id,name`, 100, reqOpts);
      for (const drive of drives) {
        ...
        try {
          ...
          while (next) {
            ...
            const page = await httpJson(auth, next, reqOpts);
            for (const item of page.value ?? []) {
              scanned++; itemsScanned++;
              if (!item.shared) continue;
              const perms = await httpJson(auth, `/drives/${drive.id}/items/${item.id}/permissions`, reqOpts).catch(() => null);
