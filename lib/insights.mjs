...
export function computeFindings(state) {
  ...
  // Values that prove external exposure — "none-observed"/"unknown" don't count.
  const EXTERNAL_SHARING = new Set(['anyone', 'existing-guests', 'new-and-existing-guests']);

  if (enabled('pol-confidential-external')) {
    for (const s of sites) {
      if (s.sensitivity !== 'General' && EXTERNAL_SHARING.has(s.externalSharing)) {
        findings.push({
...
export function buildSummary(state) {
  ...
  return {
    ...
    writeBack: !!state.settings.writeBack,
